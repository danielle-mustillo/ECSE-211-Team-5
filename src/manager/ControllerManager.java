...
/**
 * 
 * Manages the controllers. Throughout the life of the program, the controller
 * corresponding to the current state will be run It is up to each controller to
...
public class ControllerManager implements TimerListener {
	...
	public ControllerManager(Manager manager) {
		...
		this.controllers = new Controller[] { new Search(manager),
				new Recognize(manager), new Collect(manager),
				new DropOff(manager), new WallFollower(manager) };
		start();
	}
	...
	public void timedOut() {
		...
		if (state == State.SEARCH)
			controllers[0].run();
		else if (state == State.RECOGNIZE)
			controllers[1].run();
		else if (state == State.COLLECT)
			controllers[2].run();
		else if (state == State.DROP_OFF)
			controllers[3].run();
		else if (state == State.WALL_FOLLOWER)
			controllers[4].run();

	}

	/**
	 * Stops the controller timer. No more controllers will be run until start
	 * is called.
	 */
	public void stop() {
		timer.stop();
