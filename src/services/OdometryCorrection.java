...
/**
 * OdometryCorrection makes use of the {@link LinePoller}, each time a new line
...
public class OdometryCorrection implements TimerListener {
	...
	/**
	 * Max distance between points to correct (i.e. max distance between left
	...
	/**
	 * For offseting the grid lines (i.e. to move 0,0 away from the first grid
	 * intersection )
	 */
	 ...
	 * @param rightPos
	 */
	public void lineDetected(Position leftPos, Position rightPos,
			int firstSensor) {
		Sound.beep();
		/*
		 * Ensures that the point are reasonably close to each other
		 */
		if (leftPos.distanceToPoint(rightPos) > MAX_DISTANCE_BETWEEN_POINTS
				&& Math.abs(leftPos.theta - rightPos.theta) > 0.1) {
			return;
		}

		// Average the x and y of the line detected, as the robot should detect
		// both at the same time if it is going perpendicular to the grid line
		...
		if (Math.abs(Math.sin(theta)) > 0.98) {
			...
			// make sure the sensor is not close to a line parallel to direction
			// of travel
			if (Math.abs(x - line) > Settings.LS_WIDTH / 2
					+ MAX_DISTANCE_FROM_TILE_CENTER) {

				/*
				 * getClosestY returns the closest line plus the offset for the
				 * light Sensor
				 */
		 ...
		 */
			...
			// make sure the sensor is not close to a line parallel with
			// direction of travel
			if (Math.abs(y - line) > Settings.LS_WIDTH / 2
					+ MAX_DISTANCE_FROM_TILE_CENTER) {
		...
		double dTheta = Math.atan(leftPos.distanceToPoint(rightPos)
				/ Settings.LS_WIDTH);
		// adjust sign
		if (firstSensor == left) {
			dTheta *= -1;
		...
		if (Math.abs(dTheta) < MAX_CHANGE_ANGLE
	...
	public double getClosestY(double y, double theta) {
		...
		return (Math.round((y - R - yOffset) / Settings.TILE_SIZE)
				* Settings.TILE_SIZE + yOffset + R);
	}
	...
	public double getClosestX(double x, double theta) {
		...
		return (Math.round((x - R - xOffset) / Settings.TILE_SIZE)
				* Settings.TILE_SIZE + xOffset + R);
	}
